from app import db
from app.infrastructure.models import Scenario, Asset, ScenarioAsset
...
class TestScenarioAssetModelCase:
    def test_default_scenario_asset(self, default_asset, default_scenario):
        ...
        asset_from_db = db.session.scalar(
            sa.select(Asset).where(Asset.id == association.asset_id)
        )
        ...
        assert association.asset_id == asset_from_db.id
