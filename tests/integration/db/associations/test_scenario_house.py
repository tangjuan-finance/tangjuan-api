from app import db
from app.infrastructure.models import Scenario, House, ScenarioHouse
...
class TestScenarioHouseModelCase:
    def test_default_scenario_house(self, default_house, default_scenario):
        ...
        house_from_db = db.session.scalar(
            sa.select(House).where(House.id == association.house_id)
        )
        ...
        assert association.house_id == house_from_db.id
