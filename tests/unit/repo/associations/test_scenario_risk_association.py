...
from tests.unit.factories import RiskDomainFactory, ScenarioDomainFactory
...
class TestRiskRepoCase:
    ...
    def test_create_scenario_risk_assoc_through_repo(self, default_account):
        # Arrange: Create an risk and a scenario domain using the factory
        default_max_loss = 100000
        scenario = ScenarioDomainFactory(owner=default_account)
        risk = RiskDomainFactory(owner=default_account, max_loss=default_max_loss)
    ...
    def test_update_scenario_risk_assoc_through_repo(self, default_account):
    ...
    def test_get_scenario_risk_assoc_by_id_through_repo(self, default_account):
        # Arrange: Create an risk domain using the factory
        scenario_risk_from_repo = self._create_assoc(default_account)
    ...
    def test_delete_scenario_risk_assoc_through_repo(self, default_account):
        # Arrange: Create an risk domain using the factory
        scenario_risk_from_repo = self._create_assoc(default_account)

        # Act: Delete the risk domain object
        ScenarioRiskRepo.delete_by_id(scenario_risk_from_repo.id)

        # Assert: Ensure the risk record is deleted from the database
        scenario_risk_from_db = db.session.scalars(
            sa.select(ScenarioRiskRepo).where(
                (ScenarioRisk.scenario_id == scenario_risk_from_repo.scenario.id)
                & (ScenarioRisk.risk_id == scenario_risk_from_repo.risk.id)
            )
        )
