...
from tests.unit.factories import HouseDomainFactory, ScenarioDomainFactory
...
class TestHouseRepoCase:
    ...
    def test_create_scenario_house_assoc_through_repo(self, default_account):
        # Arrange: Create an house and a scenario domain using the factory
        default_interest_rate = Decimal("2.0")
        scenario = ScenarioDomainFactory(owner=default_account)
        house = HouseDomainFactory(
            owner=default_account, interest_rate=default_interest_rate
        )
        assoc_interest_rate = Decimal("7.0")
        assoc_domain = ScenarioHouseDomain(
            house=house,
            scenario=scenario,
    ...
    def test_update_scenario_house_assoc_through_repo(self, default_account):
    ...
    def test_get_scenario_house_assoc_by_id_through_repo(self, default_account):
        # Arrange: Create an house domain using the factory
        scenario_house_from_repo = self._create_assoc(default_account)
    ...
    def test_delete_scenario_house_assoc_through_repo(self, default_account):
        # Arrange: Create an house domain using the factory
        scenario_house_from_repo = self._create_assoc(default_account)

        # Act: Delete the house domain object
        ScenarioHouseRepo.delete_by_id(scenario_house_from_repo.id)

        # Assert: Ensure the house record is deleted from the database
        scenario_house_from_db = db.session.scalars(
            sa.select(ScenarioHouseRepo).where(
                (ScenarioHouse.scenario_id == scenario_house_from_repo.scenario.id)
                & (ScenarioHouse.house_id == scenario_house_from_repo.house.id)
            )
        )
