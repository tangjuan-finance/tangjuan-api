...
class TestLiabilityRepoCase:
    def test_create_liability_domain_through_repo(self):
        ...
        assert liability_from_repo.owner == liability_from_db.owner
    ...
    def test_update_liability_domain_through_repo(self):
        # Arrange: Create an liability domain using the factory
        liability = LiabilityDomainFactory()
        liability_from_repo = LiabilityRepo.create(liability)
    ...
    def test_get_liability_domain_by_id_through_repo(self):
        # Arrange: Create an liability domain using the factory
        liability = LiabilityDomainFactory()
        LiabilityRepo.create(liability)

        # Act: Update the liability domain object (before saving)
        liability_get_by_id = LiabilityRepo.get_by_id(liability.id)

        # Assert: Ensure the values match between the domain object and the saved record
        assert liability_get_by_id.id == liability.id
        assert liability_get_by_id.name == liability.name

    def test_get_liability_domain_list_through_repo(self):
        # Arrange: Create an liability domain using the factory
    ...
    def test_delete_liability_domain_through_repo(self):
        # Arrange: Create an liability domain using the factory
        liability = LiabilityDomainFactory()
        liability_from_repo = LiabilityRepo.create(liability)

        # Act: Delete the liability domain object
        LiabilityRepo.delete(liability_from_repo)
