...
class TestRiskRepoCase:
    def test_create_risk_domain_through_repo(self, default_account):
    ...
    def test_update_risk_domain_through_repo(self, default_account):
        # Arrange: Create an risk domain using the factory
        risk = RiskDomainFactory(owner=default_account)
        risk_from_repo = RiskRepo.create(risk)
        ...
        assert updated_risk.updated_at != risk_from_repo.updated_at

    def test_get_risk_domain_by_id_through_repo(self, default_account):
        # Arrange: Create an risk domain using the factory
        risk = RiskDomainFactory(owner=default_account)
        risk_from_repo = RiskRepo.create(risk)

        # Act: Update the risk domain object (before saving)
        risk_get_by_id = RiskRepo.get_by_id(risk_from_repo.id)

        # Assert: Ensure the values match between the domain object from repo create and the domain from repo get
        assert risk_get_by_id.id == risk_from_repo.id
        assert risk_get_by_id.name == risk_from_repo.name
    ...
    def test_delete_risk_domain_through_repo(self, default_account):
        ...
        RiskRepo.delete_by_id(risk_from_repo.id)
